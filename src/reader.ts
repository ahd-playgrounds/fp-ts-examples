...
const getYearsTillPension =
	(retirementAge: number): R.Reader<Env, number> =>
	({ age }) =>
		retirementAge - age;
const nameCensored =
	(replacements: RegExp): R.Reader<Env, string> =>
	({ name }) =>
...
const getYearsTillPensionT =
	(age: number): RT.ReaderTask<ServiceEnv, number> =>
	({ ageService }) =>
		pipe(
			ageService,
...
const nameCensoredT =
	(name: string): RT.ReaderTask<ServiceEnv, string> =>
	({ censorService }) =>
		pipe(
			censorService,
			T.map((censoredChars) =>
				name.replaceAll(new RegExp(`[${censoredChars.join("")}]`, "g"), "*")
			)
		);

const Services: ServiceEnv = {
	censorService: async () => ["o", "v"],
	ageService: async () => 66,
};
...
const getYearsTillPensionRTE =
	(age: number): RTE.ReaderTaskEither<ServicesEnv, string, number> =>
	({ ageService }) =>
		pipe(
			ageService,
...
const nameCensoredRTE =
	(name: string): RTE.ReaderTaskEither<ServicesEnv, string, string> =>
	({ censorService }) =>
		pipe(
			censorService,
			TE.map((censoredChars) =>
				name.replaceAll(new RegExp(`[${censoredChars.join("")}]`, "g"), "*")
			)
		);
